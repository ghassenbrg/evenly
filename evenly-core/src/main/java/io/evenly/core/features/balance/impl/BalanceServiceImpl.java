...
public class BalanceServiceImpl implements io.evenly.core.features.balance.BalanceService {
    ...
    public List<Balance> getBalanceForWorkspace(String workspaceId) {
        ...
        // Process expenses: paid_by gets +amount, participants get -amount/participantCount
        for (io.evenly.core.domain.Expense expense : expenses) {
            ...
            BigDecimal perParticipant = expense.getAmount().divide(BigDecimal.valueOf(participantCount), 2, BigDecimal.ROUND_HALF_UP);
            ...
            for (ExpenseParticipant participant : participants) {
                balances.merge(participant.getUserId(), perParticipant.negate(), BigDecimal::add); // userId is now String
            }
        }
        ...
        for (Payment payment : payments) {
            if ("COMPLETED".equals(payment.getStatus())) {
                balances.merge(payment.getPaidByUserId(), payment.getAmount().negate(), BigDecimal::add); // userId is now String
    ...
    @Override
    public SettleUpResponse getSettleUpForWorkspace(String workspaceId, String currentUserId) { // userId is now username (String)
    ...
    @Override
    public BalanceSummary getBalanceSummary(String workspaceId, String userId, LocalDate startDate, LocalDate endDate) { // userId is now username (String)
        UUID workspaceUuid = UUID.fromString(workspaceId);
